...
def process_record(row) -> list:
    ...
    if qualification not in uniqueQualificationsInDB:
        qualification = temp_qual.get(qualification, qualification)
    print(f'qualification revised   : {qualification}')
    print(f'designation             : {designation}')
    print(f'designation revised     : {temp[designation]}')
    _designation_id = df_designationMapping[temp[designation]]
...
def main():
    ...
    df = pl.read_excel(RECORDS) 

    uniqueQualificationsInCsv = list(set(df['scale_considered'].unique().to_list()))
    uniqueQualificationsInCsv = [i.strip().upper() for i in uniqueQualificationsInCsv]
    uniqueQualificationsInCsv.sort()
    # print(uniqueQualificationsInCsv)
    ...
    #     import time
    #     t1 = time.time()
    #     failed_record = process_record(row)
    #     t2 = time.time()
    #     print(f"Time taken to process record: {t2 - t1} seconds\n)")
    #     all_failed_records.extend(failed_record)

        
    # print("Total Records Failed: ", len(all_failed_records))
    # df_failedRecords = pl.DataFrame(all_failed_records)
    # df_failedRecords.to_pandas().to_excel(FAILED_RECORDS, index=False)
    ...
    with ThreadPoolExecutor(max_workers=BATCH_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                print("Unexpected thread failure:", e)
...
if __name__ == "__main__":
        # File Paths
    FAILED_RECORDS = 'D:/B Adhoc/failed_records.xlsx'
    RECORDS = 'D:\B Adhoc\bulk_upload\Digikit_bulk_main_data_1.xlsx'
